...
CREATE TABLE cvs (
  ...
  file_meta JSONB NOT NULL DEFAULT '{}', -- {name, ext, size, upload_date}
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE generations (
  ...
  diff_meta JSONB DEFAULT '{}', -- stores diff highlights and changes
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE INDEX idx_cvs_last_accessed ON cvs(last_accessed_at);
CREATE INDEX idx_generations_user_id ON generations(user_id);
CREATE INDEX idx_generations_cv_id ON generations(cv_id);
CREATE INDEX idx_generations_created_at ON generations(created_at);
CREATE INDEX idx_usage_tracking_month ON usage_tracking(current_month);
...
RETURNS void AS $$
BEGIN
  -- Delete old CVs and related data for inactive users
  DELETE FROM cvs 
  WHERE user_id IN (
  ...
  -- Delete profiles of users inactive for 5+ years
