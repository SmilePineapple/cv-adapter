    ...
    if (isAdvancedTemplate(template)) {
      ...
      if (maxPages) {
        if (metrics.estimatedPages > maxPages) {
          ...
          // For multi-page CVs, use AI to determine optimal page breaks and content distribution
          if (maxPages > 1 && maxPages >= 3) {
            console.log(`🎨 Using AI to optimize page layout for ${maxPages}-page CV`)
...
Current content length: ${metrics.totalContentLength} characters
Target: ${maxPages} pages (~${maxPages * 3000} characters)

Provide specific CSS instructions to:
1. Distribute sections evenly across ${maxPages} pages
2. Add page breaks at optimal points
3. Adjust spacing to fill empty space
4. Ensure each page has substantial content

Output format:
{
  "page_breaks": ["after_section_summary", "after_experience_job_1"],
  "spacing_adjustments": {
    "summary_line_height": 1.8,
    "experience_item_margin": 30,
    "section_margin": 50
  },
  "content_distribution": {
    "page_1": ["name", "contact", "summary"],
    "page_2": ["experience_job_1"],
    "page_3": ["experience_job_2", "skills"],
    "page_4": ["education", "certifications", "interests"]
  }
}`
              ...
              const layoutCompletion = await openai.chat.completions.create({
                ...
                messages: [
                  {
                    role: 'system',
                    content: 'You are a CV layout expert. Provide specific CSS and layout instructions.'
                  },
                  {
                    role: 'user',
              ...
              const layoutResponse = layoutCompletion.choices[0]?.message?.content
              console.log('🎨 AI Layout suggestions:', layoutResponse?.substring(0, 300))

              if (layoutResponse) {
                const layout = JSON.parse(layoutResponse)
                
                // Apply AI-suggested spacing adjustments
                const spacingStyle = `<style>
                  body { line-height: ${layout.spacing_adjustments?.summary_line_height || 1.8} !important; }
                  .section { margin-bottom: ${layout.spacing_adjustments?.section_margin || 50}px !important; }
                  .experience-item { margin-bottom: ${layout.spacing_adjustments?.experience_item_margin || 30}px !important; }
                  .section-header { margin-bottom: 25px !important; }
                  .education-item { margin-bottom: 25px !important; }
                  .skill-tag { margin-bottom: 15px !important; }
                  .hobby-item { margin-bottom: 18px !important; }
                  p, li, .description { margin-bottom: 10px !important; }
                  .header { padding-bottom: 40px !important; }
                  
                  /* Add page breaks after specified sections */
                  ${layout.page_breaks?.includes('after_section_summary') ? '.summary { page-break-after: always; }' : ''}
                  ${layout.page_breaks?.includes('after_experience_job_1') ? '.experience-item:nth-child(1) { page-break-after: always; }' : ''}
                </style>`
                
                html = html.replace('</head>', `${spacingStyle}</head>`)
                console.log('🎨 Applied AI layout suggestions')
              }
            } catch (error) {
              console.error('❌ AI layout optimization failed, falling back to standard spacing:', error)
              
              // Fallback to standard spacing expansion
              if (metrics.estimatedPages < maxPages) {
          ...
          } else if (maxPages > 1 && metrics.estimatedPages < maxPages) {
            // Standard spacing expansion for 2-page CVs
